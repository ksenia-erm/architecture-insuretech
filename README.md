...
Для лимита 10 запросов в минуту на IP используем параметры `limit_req_zone` + `limit_req` и директива `limit_req_status` 429

[Измененный конфиг nginx](Task06/nginx.conf)

**Пояснения**:
- `limit_req_zone` в http‑блоке создаёт общую зону `per_ip_10rpm` и задаёт лимит 10 запросов в минуту на `$binary_remote_addr` (IP-адрес клиента)
- `limit_req` в `location` / применяет ограничение; `burst=0` значит, что лишние запросы сразу отклоняются
- `limit_req_status` 429 заставляет Nginx отдавать `429 Too Many Requests` при превышении лимита
